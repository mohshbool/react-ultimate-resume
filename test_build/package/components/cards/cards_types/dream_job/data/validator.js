...
var DreamJobValidationSchema = function DreamJobValidationSchema(formatMessage) {
  return Yup.object({
    ...
    salary: Yup.number().when('contractTypes', {
      is: function is(contractType) {
        return (0, _has_only_freelance_contract.hasOnlyFreelanceContract)(contractType);
      },
      then: Yup.number().nullable().notRequired(),
      otherwise: Yup.number().min(1, formatMessage(_validation_translations.validationTranslations.minNumber, {
        min: 1
      }))
    }),
    averageDailyRate: Yup.number().when('contractTypes', {
      ...
      then: Yup.number().min(1, formatMessage(_validation_translations.validationTranslations.minNumber, {
        min: 1
      })),
